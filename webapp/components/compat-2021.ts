...
class Compat2021DataManager {
  ...
  stableDatatables!: Map<string, google.visualization.DataTable>;
  experimentalDatatables!: Map<string, google.visualization.DataTable>;
  stableBrowserVersions!: string[][];
  experimentalBrowserVersions!: string[][];
  ...
  async _loadCsv(label: 'stable' | 'experimental'): Promise<void> {
    ...
    csvLines.forEach(line => {
      ...
      const newRows = new Map(
        features.map(feature => {
          return [feature, [date] as (Date | number | string)[]];
        })
      );
  ...
  createTooltip(browser: string, version: string, score: string): string {
...
@customElement('compat-2021')
export class Compat2021 extends LitElement {
  @property({type: Boolean}) embedded = false;
  @property({type: Boolean}) useWebkitGTK = false;
  @property({type: Boolean}) stable = false;
  @property({type: String}) feature = SUMMARY_FEATURE_NAME;
  @property({type: Object}) dataManager = new Compat2021DataManager();
  ...
  updated(changedProperties: Map<string, any>) {
    if (
      ...
      changedProperties.has('useWebkitGTK') ||
  ...
  experimentalButtonClass(stable: boolean) {
    return stable ? 'unselected' : 'selected';
  }

  stableButtonClass(stable: boolean) {
  ...
  getTestListTextVisibility(feature: string) {
    return FEATURES.includes(feature) ? 'visible' : 'hidden';
  }

  getTestListHref(feature: string) {
  ...
  render() {
    return html`
      ...
      <compat-2021-summary ?stable=${this.stable}></compat-2021-summary>
      <p>
      ...
      <p>
        ...
        each feature, filtered by believed importance to web developers. The
        results shown here are from
        ${this.stable
      ...
      <fieldset>
        ...
        <div class="channel-area">
          ...
          <paper-button
            class=${this.experimentalButtonClass(this.stable)}
            raised
            @click=${this.clickExperimental}
            >Experimental</paper-button
          >
          <paper-button
            class=${this.stableButtonClass(this.stable)}
            raised
            @click=${this.clickStable}
            >Stable</paper-button
          >
        </div>
      ...
      <div
        id="testListText"
        style="visibility: ${this.getTestListTextVisibility(this.feature)}"
      >
        The score for this component is determined by pass rate on
        <a href=${this.getTestListHref(this.feature)} target="_blank"
          >this set of tests</a
        >. The test suite is never complete, and improvements are always
        welcome. Please contribute changes to
        <a href="https://github.com/web-platform-tests/wpt" target="_blank"
          >WPT</a
        >
        and then
        <a
          href="https://github.com/web-platform-tests/wpt.fyi/issues/new?title=[compat2021]%20Add%20new%20tests%20to%20dashboard&body="
          target="_blank"
          >file an issue</a
        >
...
@customElement('compat-2021-summary')
export class Compat2021Summary extends LitElement {
  @property({type: Boolean}) stable = false;
  ...
  updateSummaryTitles() {
    const titleDivs =
      this.shadowRoot!.querySelectorAll('.summary-browser-name');
    const titles = this.stable ? STABLE_TITLES : EXPERIMENTAL_TITLES;
    for (let i = 0; i < titleDivs.length; i++) {
      (titleDivs[i] as HTMLElement).innerText = titles[i];
    }
  }

  async updateSummaryScores() {
    const scores = await this.calculateSummaryScores(this.stable);
    const numbers = this.shadowRoot!.querySelectorAll('.summary-number');
    const tooltips = this.shadowRoot!.querySelectorAll('.summary-tooltip');
    for (let i = 0; i < scores.length; i++) {
      (numbers[i] as HTMLElement).innerText = scores[i].total.toString();
      (numbers[i] as HTMLElement).style.color = this.calculateColor(
        scores[i].total
      );
  ...
  async calculateSummaryScores(
    ...
    const scores: {total: number; breakdown: Map<string, number>}[] = [
    ...
    for (const line of csvLines) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        // definitely there.
        const contribution = Math.floor(parseFloat(parts[i]) * 20);
...
@customElement('compat-2021-feature-chart')
export class Compat2021FeatureChart extends LitElement {
  @property({type: Object}) dataManager!: Compat2021DataManager;
  @property({type: Boolean}) stable = false;
  ...
  updated(changedProperties: Map<string, any>) {
    if (changedProperties.has('feature') || changedProperties.has('stable')) {
      this.updateChart(this.feature, this.stable);
    }
  }

  async updateChart(feature: string, stable: boolean) {
    ...
    chart.setAction({
      ...
      action: () => {
        ...
        const browserIdx = Math.floor(column / 2);

        const version = browserVersions[browserIdx][row];
        ...
        if (browserIdx === 1) {
          if (stable) {
            window.open(
              this.getFirefoxStableChangelogUrl(lastVersion, version)
            );
          } else {
            (
              this.shadowRoot!.querySelector(
                '#firefoxNightlyDialog'
              ) as any
            ).open();
          }
          return;
        ...
        (this.shadowRoot!.querySelector('#safariDialog') as any).open();
      },
    });
  ...
  getChromeChangelogUrl(fromVersion: string, toVersion: string) {
  ...
  getFirefoxStableChangelogUrl(fromVersion: string, toVersion: string) {
  ...
  clickFirefoxNightlyDialogGoButton() {
    const fromSha = (
      this.shadowRoot!.querySelector('#firefoxNightlyDialogFrom') as any
    ).value;
    const toSha = (
      this.shadowRoot!.querySelector('#firefoxNightlyDialogTo') as any
    ).value;
    ...
    return html`
      ...
      <paper-dialog with-backdrop id="firefoxNightlyDialog">
        ...
        <div>
          ...
          <a
            href="https://hg.mozilla.org/mozilla-central/firefoxreleases"
            target="_blank"
            >release page</a
          >, enter them below, and click "Go".
          <paper-input
            id="firefoxNightlyDialogFrom"
            label="From revision"
          ></paper-input>
          <paper-input
            id="firefoxNightlyDialogTo"
            label="To revision"
          ></paper-input>
        </div>
        ...
        <div class="buttons">
          ...
          <paper-button
            dialog-confirm
            @click=${this.clickFirefoxNightlyDialogGoButton}
            >Go</paper-button
          >
        </div>
      </paper-dialog>
      ...
      <paper-dialog with-backdrop id="safariDialog">
        ...
        ${this.stable
          ? html`
              <div>
                Stable releases of Safari do not publish changelogs, but some
                insight may be gained from the
                <a
          ...
          : html`
              <div>
                For Safari Technology Preview releases, release notes can be
                found on the
                <a href="https://webkit.org/blog/" target="_blank"
                  >WebKit Blog</a
                >. Each post usually contains a revision changelog link - look
                for the text "This release covers WebKit revisions ...".
              </div>
            `}
